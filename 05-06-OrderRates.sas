...
ods graphics / width=480px height=720px;
...
data Relative/view=Relative;
set StatePop;
Rel = PopChange2020 / Pop2020;
label Rel = "Relative Change in State Population";
run;

title "Relative Change in State Populations (2020)";
proc sgplot data=Relative;
   hbar StateName / response=Rel categoryorder=respasc; /* or respdesc */
   yaxis display=(nolabel) colorbands=even     /* faint alternating bands */
                           valueattrs=(size=8) /* small font for values */
                           fitpolicy=none;     /* do not thin labels */
   xaxis grid;
